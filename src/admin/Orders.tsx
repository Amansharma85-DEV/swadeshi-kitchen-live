...
import { Search, Eye, Phone, MessageSquare, Printer, X, MapPin, User, Mail, FileText, Calendar, CreditCard, Package } from 'lucide-react';
...
export default function Orders() {
  ...
  const loadOrders = async () => {
    ...
    if (apiOrders && apiOrders.length > 0) {
      const mapped: any[] = apiOrders.map((o: any) => ({
  ...
  const filteredOrders = orders.filter(o => 
    ...
    o.customer.address.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white">Orders Manager</h1>
          <p className="mt-1 text-slate-500">Track and manage live customer orders from AWS database.</p>
        </div>
      </div>
